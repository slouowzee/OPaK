<x-app-layout>
    ...
    <div class="py-12 px-4">
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-sm font-black text-gray-500 uppercase tracking-[0.2em]">Abonnements de {{ $user->name }}</h2>
        ...
        <div class="space-y-4">
            @foreach($users as $following)
                <div class="bg-gray-900/40 border border-gray-800 p-6 rounded-2xl flex items-center justify-between hover:bg-gray-900/60 transition-all group">
                    <a href="{{ route('profile.wall', ['user' => $following->name]) }}" class="flex items-center gap-4 flex-1">
                        <div class="w-12 h-12 bg-gray-800 rounded-xl overflow-hidden flex-shrink-0 border border-gray-700">
                            @if($following->avatar)
                                <img src="{{ asset('storage/' . $following->avatar) }}" class="w-full h-full object-cover" alt="">
                            @else
                        ...
                        <div>
                            <h3 class="font-black text-white group-hover:text-blue-400 transition-colors">{{ $following->name }}</h3>
                            <p class="text-gray-600 text-xs font-mono">@ {{ strtolower(str_replace(' ', '', $following->name)) }}</p>
                        </div>
                    </a>
            ...
            @if($users->isEmpty())
                <div class="bg-gray-900/20 border-2 border-dashed border-gray-800 p-12 rounded-3xl text-center">
                    <p class="text-gray-500 text-lg font-medium">Aucun abonnement pour le moment.</p>
                </div>
            @endif
        </div>
